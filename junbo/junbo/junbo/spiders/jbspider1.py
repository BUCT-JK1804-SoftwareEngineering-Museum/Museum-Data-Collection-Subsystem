...
class Jbspider1Spider(scrapy.Spider):
    name = 'jbspider1'
    allowed_domains = ['www.jb.mil.cn/']
    start_urls = ['http://www.jb.mil.cn/was/web/search?token=14.1499419140318.94&channelid=207631&perpage=30']
    ...
    def parse(self, response):
        ...
        gjds = response.xpath("//div[@class='raAppList']/ul/li")
        # print('#' * 40)
        # print(gjds)
        # print('#' * 40)
        for gjd in gjds:
            # 爬取名字和图片地址
            name = gjd.xpath(".//span/text()").get()
            # img = gjd.xpath(".//img/@src").getall()
            # img = gjd.xpath(".//script[2]").get()
            img_url = gjd.xpath(".//a/@href").get()
    ...
    def parse_detail(self, response):
        ...
        era = response.xpath("//div[@class='TRS_Editor']/p[2]//text()").get()
        # era= "".join(era).strip()
