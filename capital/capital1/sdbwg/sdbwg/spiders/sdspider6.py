...
class Sdspider6Spider(scrapy.Spider):
    name = 'sdspider6'
    ...
    def parse(self, response):
        ...
        #设置“下一页”
        next_url = self.base_url+"jyq_"+str(self.x)+".htm"
        #测试网络跳转情况
        self.x += 1
        # print('#' * 40)
        # print(next_url)
    ...
    def parse_detail(self,response):
        ...
        introduction = response.xpath("//div[@style='TEXT-ALIGN: left']/p[1]/text()").getall()
        introduction= "".join(introduction).strip()
        era = response.xpath("//div[@style='TEXT-ALIGN: left']/p[1]/text()").getall()
        era = "".join(era).strip()
