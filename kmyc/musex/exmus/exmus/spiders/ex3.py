...
class Ex3Spider(scrapy.Spider):
    ...
    start_urls = ['http://www.kmycjng.com/list?cid=26']
    ...
    def parse(self, response):
        qtqs = response.xpath("//div[@class='sy_jccl_twlb']/ul/li")
        for qtq in qtqs:
            # 爬取名字和图片地址
            name = qtq.xpath(".//div[@class='sy_tit']//text()").get()
            img_url = qtq.xpath(".//div[@class='sy_img']/img/@src").get()
            # print('#' * 40)
            # print(name)
            # print(img_url)
            # print('#' * 40)
            yield scrapy.Request(self.base_url + img_url, callback=self.parse_detail, dont_filter=True,
                                 meta={"name": name, "ima":img_url})
        # 设置“下一页”
        # j = self.i
    ...
    def parse_detail(self,response):
        ...
        id = str(self.id_num + self.mus_ida * 100000 + 30000)
        mus_id = str(self.mus_ida)
        mus_name = self.mus_name_num
        image = self.base_url + response.meta["ima"]
        time = self.timmme
        info = "null"
