...
class F1Spider(scrapy.Spider):
    ...
    start_urls = ['http://www.kmycjng.com/list?cid=29']
    ...
    def parse(self, response):
        ...
        # 设置“下一页”
        j = self.i
        next_url = self.fur_url + str(j) + "&cid=29"
    ...
    def parse_detail(self, response):
        ...
        ima = response.xpath("//div[@class='infocontent']//img/@src").get()
        flag_ima = response.xpath("//div[@class='infocontent']//img/@border").get()
        if flag_ima == "0":
            image = ima
        else:
            image = self.base_url + ima
        ...
        era = "null"
        introduction = "null"
        item = CollItem(id=id, mus_id=mus_id, name=name, mus_name=mus_name, image=image, era=era,
