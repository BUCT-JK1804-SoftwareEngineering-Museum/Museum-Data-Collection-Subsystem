...
class Lj2Spider(scrapy.Spider):
    name = 'lj2'
    allowed_domains = ['http://www.hljmuseum.com/']
    start_urls = ['http://www.hljmuseum.com/cpgz/cpxs/lsww/']
    ...
    def parse(self, response):
        ...
        for qtq in qtqs:
            ...
            yield scrapy.Request(self.base_url+fur_url, callback=self.parse_detail, dont_filter=True,meta={"name":name, "image":img_url})

        j = self.i
        # next_url = self.base_url+"/dzzp/qtq/index_"+str(j)+".html"
        next_url = "http://www.hljmuseum.com/system/more/cpgz/cpxs/lsww/index/page_0"+ str(j) +".html"
        # next_url = qtqs.xpath("//div[@class='redh']/a["+str(j)+"]/@href").get()
    ...
    def parse_detail(self,response):
        ...
        era= "见名称"
        introduction = response.xpath("//div[@class='duanluo']/p//text()").getall()
