...
class Exspider4Spider(scrapy.Spider):
    name = 'exspider4'
    allowed_domains = ['http://www.bjqtm.com']
    start_urls = ['http://www.bjqtm.com/clzl/szzt/']
    ...
    def parse(self, response):
        ...
        for zl in zls:
            ...
            pic_url = zl.xpath(".//div[@class='pos-r img']/style").get()
                # .strip('background:url(').strip(') no-repeat center center/cover')

            # pic = zl.xpath(".//div[@class='pos-r img']/style")
            # pic_irl = str(pic).replace("background:url('", "")
            # pic_url = str(pic_irl).replace("') no-repeat center center/cover", "")

            yield scrapy.Request(self.base_url, callback=self.parse_detail, dont_filter=True,
                                 meta={"name": name,"pic_url":pic_url})
            # yield scrapy.Request(callback=self.parse_detail, dont_filter=True,
            #                      meta={"name": name})
    ...
    def parse_detail(self, response):
        ...
        exh_picture = self.base_url + response.meta["pic_url"]
